...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_redirects_to_all_borrowed_book_list_on_success(self):
        ...
        response = self.client.post(reverse('catalog:renew-book-librarian', kwargs={'pk':self.test_bookinstance1.pk,}), {'renewal_date':valid_date_in_future})
...
class AuthorCreateViewTest(TestCase):
    def setUp(self):
        ...
        test_user1 = User.objects.create_user( username='test_user1', password='some_password')
    ...
    def test_forbidden_if_logged_in_but_not_correct_permission(self):
        _ = self.client.login(username='test_user1', password='some_password')
        response = self.client.get(reverse('catalog:author-create'))
        self.assertEqual(response.status_code, 403)

    def test_uses_correct_template(self):
        _ = self.client.login(username='test_user', password='some_password')
        response = self.client.get(reverse('catalog:author-create'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'catalog/author_form.html')
    
    def test_redirect_to_author_list(self):
        _ = self.client.login(username='test_user', password='some_password')
        response = self.client.post(reverse('catalog:author-create'), {'first_name': 'test_first_name', 'last_name': 'test_last_name'})
        self.assertEqual(response.status_code, 302)
